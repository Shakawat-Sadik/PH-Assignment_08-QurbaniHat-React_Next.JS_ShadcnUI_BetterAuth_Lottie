...
const glassButtonVariants = cva(cn(
  ...
  variants: {
    variant: {
      default: cn(
        "bg-white/20 backdrop-blur-xl border border-white/30 text-foreground",
      ...
      primary: cn(
        "bg-linear-to-r from-cyan-500/80 via-emerald-400/80 to-emerald-500/80",
        "backdrop-blur-xl border border-white/30 text-foreground",
  ...
  ({ className, variant, asChild = false, size, glowEffect = false, children, ...props }, ref) => {
    ...
    return (
      <div className="relative inline-block">
        {glowEffect && (
          <div
            className="absolute -inset-1 rounded-xl bg-linear-to-r from-cyan-500/40 via-emerald-400/40 to-emerald-500/40 blur-lg opacity-70 transition-opacity group-hover:opacity-100" />
        )}
        <Comp
